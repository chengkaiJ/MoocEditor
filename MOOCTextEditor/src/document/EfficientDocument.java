...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for (int i = 0; i<tokens.size(); i++) {
			if(i+1==tokens.size()) { //this token is the at last so should be a sentence
				numSentences++;
			...
			else if(isWord(tokens.get(i))) {//this token is a word
				numWords++;
				numSyllables+=countSyllables(tokens.get(i));
			}
			else {//this token is punctuations and is not the last one(remember 7.5.)
				if(isWord(tokens.get(i+1)) && i!=0) {//next one is not a punctuation
					numSentences++;
				}
			}
		}	
