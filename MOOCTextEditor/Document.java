...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (int i=0; i<word.length(); i++) {
			char tested = word.charAt(i);
		
			
		}*/
		
		List<String> syllables = new BasicDocument(word).getTokens("[aeiouyAEIOUY]+");
		int numofSy = syllables.size();
		//System.out.println(word+ ": " + syllables);
		
		//if the word last letter is e && last vowel is e && there is more than one vowel
		if(syllables.get(numofSy-1).equals("e")&&numofSy>1&&word.charAt(word.length()-1)=='e') {
			numofSy = numofSy-1;
	...
	public double getFleschScore()
	{
		...
		double fleschScore = 206.835-1.015*((double)getNumWords()/getNumSentences())-84.6*((double)getNumSyllables()/getNumWords());
		//System.out.println(getNumSyllables()/getNumWords());
	    return fleschScore;
	}
